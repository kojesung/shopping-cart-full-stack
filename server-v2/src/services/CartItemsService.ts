...
export const buildCartItems = async (userId: string): Promise<CartItem[]> => {
    ...
    return records
        ...
        .map(
            (record) => new CartItem(productById.get(record.productId) as Product, record.quantity, record.checkStatus)
        );
};
...
const findMissingCartItemFields = (cartItem: { productId?: unknown; quantity?: unknown }): FieldError[] => {
