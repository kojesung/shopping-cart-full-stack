...
const computeOrderCheckPayInfo = async (order: OrderCheckRecord | null, userId: string) => {
  ...
  const { orderDiscountAmount, shippingDiscountAmount } =
    await getSelectedCouponDiscountSummary(userId);
...
export const selectRemoteArea = async (
  ...
  const order = await findOrderOrThrow(userId);
  const updated = await orderCheckRepository.setRemoteAreaCheckStatus(
    order,
